...
use topology_checker::{
    geometries_to_file,
    rules::Rules,
    rules::{must_not_intersect, there_are_no_dangles, must_not_overlap},
    VectorDataset,
};
...
fn main() {
    ...
    let polygons: Vec<geo::MultiPolygon> = must_not_overlap(geometries);
    // let points = there_are_no_dangles(geometries);
    ...
    geometries_to_file(polygons, "./assets/polygons_overlap.shp");
